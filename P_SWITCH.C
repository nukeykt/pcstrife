...
//
void P_SpawnBrokenGlass(line_t* line)
{
    fixed_t x;
    fixed_t y;
    int i;
    ...
    x = (line->v2->x + line->v1->x) / 2;
    y = (line->v2->y + line->v1->y) / 2;
    x += ((line->frontsector->soundorg.x - x) / 5);
    y += ((line->frontsector->soundorg.y - y) / 5);

    for (i = 0; i < 7; i++)
    {
        glass = P_SpawnMobj(x, y, ONFLOORZ, MT_JUNK);
        glass->z += (24 * FRACUNIT);
        ...
        glass->momx = FixedMul((P_Random() & 3) << FRACBITS, finecosine[an]);
        glass->momy = FixedMul((P_Random() & 3) << FRACBITS, finecosine[an]);
  ...
  int 		useAgain )
{
    ...
    for (i = 0; i < numswitches * 2; i++)
    {
        if (switchlist[i] == texTop)
        {
            ...
            if (alphSwitchList[i / 2].sound)
                sound = alphSwitchList[i / 2].sound;
...
//
void P_MoveWall(line_t *line, mobj_t *thing)
{
    vertex_t *v2;
    vertex_t *v1;
    fixed_t x;
...
{               
    ...
    case 211:
        ...
        {
            sprintf(usemessage, "voc%i", line->tag);
    ...
    case 209:
        ...
        {
            sprintf(usemessage, "You need the chalice!");
    ...
    case 226:
        ...
        if (EV_DoFloor(line, lowerFloor))
        {
            ...
            P_ChangeSwitchTexture(line, 0);
            sprintf(usemessage, "Congratulations! You have completed the training area.");
            thing->player->message = usemessage;
        }
        break;
    ...
    case 154:
        ...
        else if (EV_DoPlat(line, downWaitUpStay, 0))
            P_ChangeSwitchTexture(line, 1);
        break;
    ...
    case 233:
        ...
        P_ChangeSwitchTexture(line, 1);
        I_StartVoice("voc70");
        lump = W_CheckNumForName("log70");
        if (lump > 0)
        {
            strncpy(mission_objective, W_CacheLumpNum(lump, PU_CACHE),
                OBJECTIVE_LEN);
            sprintf(usemessage, "Incoming Message from BlackBird...");
            thing->player->message = usemessage;
        }
    ...
    case 234:
        ...
        if (!(thing->player->questflags & QF_QUEST3)) // QUEST3 == Irale
        {
            ...
            sprintf(usemessage, "That doesn't seem to work!");
            thing->player->message = usemessage;
        }
