...
void P_LoadThings (int lump)
{
    ...
    memset(mapmarkpoints, sizeof(mapthing_t)*AM_NUMMARKPOINTS, 0);
    data = W_CacheLumpNum(lump, PU_STATIC);
    ...
    for (i = 0; i<numthings; i++, mt++)
    {
        ...
        if (mt->type >= 118 && mt->type < 128)
        {
            // initialize riftSpots
            int riftSpotNum = mt->type - 118;
            mt->type = 1;
            riftSpots[riftSpotNum] = *mt;
        }
        else if (mt->type >= 9001 && mt->type < 9011)
        {
            int markNum = mt->type - 9001;
            mt->type = 1;
            mapmarkpoints[markNum] = *mt;
        }
        else
            P_SpawnMapThing(mt);
    }
