...
//
void P_WakeUpThing(mobj_t* puncher, mobj_t* bystander)
{
...
void P_NewRandomDir(mobj_t* actor)
{
    ...
    if (P_Random() & 1)
    {
        ...
        for (dir = DI_EAST; dir <= DI_SOUTHEAST; dir++)
        {
            if (dir != omovedir)
            {
                actor->movedir = dir;
    ...
    else
    {
        ...
        for (dir = DI_SOUTHEAST; dir != (DI_EAST - 1); dir--)
        {
...
void A_FriendLook(mobj_t* actor)
{
    ...
    if(P_Random() < 30)
    {
        P_SetMobjState(actor, (P_Random() & 1) + actor->info->spawnstate + 1);
    }
...
void A_TemplarMauler(mobj_t* actor)
{
    ...
    for(i = 0; i < 10; i++)
    {
        ...
        damage = (P_Random() & 4) * 2;
        angle = actor->angle + ((P_Random() - P_Random()) << 19);
        slope = ((P_Random() - P_Random()) << 5) + slope;
        P_LineAttack(actor, angle, MISSILERANGE + 64 * FRACUNIT, slope, damage);
    }
}
...
void A_Sigil_A_Action(mobj_t* actor)
{
    ...
    actor->momx += ((P_Random() & 3) - (P_Random() & 3)) << FRACBITS;
    actor->momy += ((P_Random() & 3) - (P_Random() & 3)) << FRACBITS;
    x = 50*FRACUNIT * ((P_Random() & 3) - (P_Random() & 3)) + actor->x;
    y = 50*FRACUNIT * ((P_Random() & 3) - (P_Random() & 3)) + actor->y;
...
void A_SpectreCAttack(mobj_t* actor)
{
    ...
    z = actor->z + 32 * FRACUNIT;
    mo = P_SpawnMobj(actor->x, actor->y, z, MT_SIGIL_A_ZAP_RIGHT);
    mo->momz = -(18*FRACUNIT);
    mo->target = actor;
    mo->health = -2;
    mo->tracer = actor->target;
...
void A_SpawnSparkPuff(mobj_t* actor)
{
    ...
    x = (10*FRACUNIT) * ((P_Random() & 3) - (P_Random() & 3)) + actor->x;
    y = (10*FRACUNIT) * ((P_Random() & 3) - (P_Random() & 3)) + actor->y;
...
void A_ProgrammerDie(mobj_t* actor)
{
    ...
    an = ((P_Random() - P_Random()) << 22) + actor->angle + ANG180;
    mo->angle = an;

    an = an >> ANGLETOFINESHIFT;
    actor->momx = FixedMul(mo->info->speed, finecosine[an]);
    actor->momy = FixedMul(mo->info->speed, finesine[an]);
...
void A_InqTossArm(mobj_t* actor)
{
    ...
    an = ((P_Random() - P_Random()) << 22) + actor->angle - ANG90;
    mo->angle = an;
...
fixed_t entity_pos_x = 0;
fixed_t entity_pos_y = 0;
fixed_t entity_pos_z = 0;
...
void P_ThrustMobj(mobj_t *actor, angle_t angle, fixed_t force)
{
    ...
    actor->momx += FixedMul(force, finecosine[an]);
    actor->momy += FixedMul(force, finesine[an]);
}
...
void A_EntityDeath(mobj_t* actor)
{
    ...
    // Subentity One
    an = actor->angle >> ANGLETOFINESHIFT;
    x = FixedMul(dist, finecosine[an]);
    y = FixedMul(dist, finesine[an]);
    subentity = P_SpawnMobj(entity_pos_x + x,
                            entity_pos_y + y,
...
void A_CrystalExplode(mobj_t* actor)
{
    ...
    for(i = 0; i < 8; i++)
    {
        ...
        rubble->momx = ((P_Random() & 15) - (P_Random() & 7)) << FRACBITS;
        rubble->momy = ((P_Random() & 7) - (P_Random() & 7)) << FRACBITS;
...
//
void P_FreePrisoners(mobj_t *actor)
{
    int i;
...
//
void P_DestroyConverter(mobj_t *actor)
{
    int i;
...
void A_NodeChunk(mobj_t* actor)
{
    ...
    mo->momx = ((P_Random() & 15) - (P_Random() & 7)) << FRACBITS;
    mo->momy = ((P_Random() & 7) - (P_Random() & 15)) << FRACBITS;
    mo->momz = (P_Random() & 15) << FRACBITS;
}
...
void A_HeadChunk(mobj_t* actor)
{
    ...
    mo->momx = ((P_Random() & 7) - (P_Random() & 15)) << FRACBITS;
    mo->momy = ((P_Random() & 15) - (P_Random() & 7)) << FRACBITS;
...
void A_BodyParts(mobj_t* actor)
{
    ...
    mo->momx = FixedMul((P_Random() & 15) << FRACBITS, finecosine[an]);
    mo->momy = FixedMul((P_Random() & 15) << FRACBITS, finesine[an]);
    mo->momz = (P_Random() & 15) << FRACBITS;
}
