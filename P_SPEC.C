...
void P_InitPicAnims (void)
{
    ...
    for (i = 0; animdefs[i].istexture != -1; i++)
    {
        if (i == MAXANIMS)
        {
            I_Error("max animated lines (%i) reached: %s",
                i, animdefs[i].startname);
        }
...
terraintype_e P_GetTerrainType(mobj_t* mobj)
{
    ...
    s = mobj->subsector->sector;

    if (mobj->z <= s->floorheight)
    {
        i = 0;
        while (terraintypes[i].type != FLOOR_END)
        {
            if (terraintypes[i].num == s->floorpic)
  ...
  mobj_t*   thing )
{
    ...
    case 202:
        ...
        // load voice
        sprintf(crosslinestr, "voc%i", line->tag);
        I_StartVoice(crosslinestr);

        // load objective
        sprintf(crosslinestr, "log%i", line->tag);
        lump = W_CheckNumForName(crosslinestr);
        if (lump > 0)
        {
    ...
    case 215:
        ...
        // start voice
        sprintf(crosslinestr, "voc%i", line->tag / 100);
        I_StartVoice(crosslinestr);

        // give objective
        sprintf(crosslinestr, "log%i", line->tag / 100);
        lump = W_CheckNumForName(crosslinestr);
        if (lump > 0)
        {
            strncpy(mission_objective, W_CacheLumpNum(lump, PU_CACHE),
    ...
    case 145:
        ...
        thing->momx = thing->momy = thing->momz = 0;
        {
            ...
            sprintf(crosslinestr, "Entering%s",
                mapnames[map - 1] + 8);
            ...
            if (netgame && deathmatch)
            {
                ...
                {
                    sprintf(crosslinestr, "%d min left",
