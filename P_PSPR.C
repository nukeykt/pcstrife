  ...
  pspdef_t* psp ) 
{
    ...
    damage = (P_Random() & (player->stamina / 10 + 7)) * (player->stamina / 10 + 2);
...
void A_FireMissile(player_t* player, pspdef_t* psp)
{
    ...
    angleshift = 19 - (player->accuracy * 5) / 100;
    an = player->mo->angle;
    player->mo->angle += (P_Random() - P_Random()) << angleshift;
    P_SetMobjState(player->mo, S_PLAY_06);
    player->ammo[weaponinfo[player->readyweapon].ammo]--;
...
void A_FireGrenade(player_t* player, pspdef_t* pspr)
{
    ...
    mo->momz = FixedMul(mo->info->speed, (player->pitch<<FRACBITS) / 160) + (8*FRACUNIT);
    if(mo->info->seesound)
        S_StartSound(mo, mo->info->seesound);

    an = (player->mo->angle >> ANGLETOFINESHIFT);
    radius = mobjinfo[type].radius + player->mo->info->radius + (4 * FRACUNIT);

    mo->x += FixedMul(radius, finecosine[an]);
    mo->y += FixedMul(radius, finesine[an]);
...
void A_FireElectricBolt(player_t* player, pspdef_t* pspr)
{
    ...
    angleshift = 18 - (player->accuracy * 5) / 100;
    an = player->mo->angle;
    player->mo->angle += (P_Random() - P_Random()) << angleshift;
    player->ammo[weaponinfo[player->readyweapon].ammo]--;
...
void A_FirePoisonBolt(player_t* player, pspdef_t* pspr)
{
    ...
    angleshift = 18 - (player->accuracy * 5) / 100;
    an = player->mo->angle;
    player->mo->angle += (P_Random() - P_Random()) << (18 - (player->accuracy * 5 / 100));
    player->ammo[weaponinfo[player->readyweapon].ammo]--;
  ...
  boolean   accurate )
{
    ...
    angleshift = 20 - (mo->player->accuracy * 5) / 100;
    angle = mo->angle;

    if (!accurate)
        angle += (P_Random() - P_Random()) << angleshift;
...
void A_FireSigil(player_t* player, pspdef_t* pspr)
{
    ...
    case 4:
        ...
        if (!linetarget)
        {
            ...
            mo->momz += FixedMul(mo->info->speed, finesine[an]);
        }
        break;
