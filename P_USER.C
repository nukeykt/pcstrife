...
char useinventorymsg[44];    // villsa [STRIFE]
    ...
    */
    ...
    if (cmd->buttons & BT_CHANGE)
    {
        ...
        if (newweapon == wp_elecbow
            && player->weaponowned[wp_poisonbow]
            && player->readyweapon == wp_elecbow
            && player->ammo[weaponinfo[wp_poisonbow].ammo])
        {
        ...
        if (newweapon == wp_hegrenade
            && player->weaponowned[wp_wpgrenade]
            && player->readyweapon == wp_hegrenade
            && player->ammo[weaponinfo[wp_wpgrenade].ammo])
        {
        ...
        if (newweapon == wp_mauler
            && player->weaponowned[wp_torpedo]
            && player->readyweapon == wp_mauler
            && player->ammo[am_cell] >= 30)
        {
    ...
    if (player->powers[pw_targeter])
    {
        ...
        else if (player->powers[pw_targeter] < 5 * TICRATE)
        {
...
char* P_RemoveInventoryItem(player_t *player, int slot, int amount)
{
    ...
    if (!player->inventory[slot].amount)
    {
        ...
        if (player->inventorycursor && slot <= player->inventorycursor)
        {
...
void P_DropInventoryItem(player_t* player, int sprite)
{
    ...
    if (item->amount)
    {
        ...
        angle = (player->mo->angle + ((P_Random() - P_Random()) << 18)) >> ANGLETOFINESHIFT;

        if (angle < 1474 && angle >= 574)
            angle = 1474;
        else if (angle < 3522 && angle >= 2622)
            angle = 3522;
        else if (angle < 5570 && angle >= 4670)
            angle = 5570;
        else if (angle < 7618 && angle >= 6718)
            angle = 7618;

        mo = player->mo;
        dist = mobjinfo[type].radius + mo->info->radius + (4 * FRACUNIT);

        x = mo->x + FixedMul(dist, finecosine[angle]);
        y = mo->y + FixedMul(dist, finesine[angle]);
        z = mo->z + (10 * FRACUNIT);
        ...
        if (P_CheckPosition(mobjitem, x, y))
        {
            ...
            mobjitem->momx = FixedMul((5 * FRACUNIT), finecosine[angle]) + mo->momx;
            mobjitem->momy = FixedMul((5 * FRACUNIT), finesine[angle]) + mo->momy;
...
boolean P_TossDegninOre(player_t* player)
{
    ...
    if (P_CheckPosition(ore, x, y))
    {
        ...
        ore->momx = FixedMul((5 * FRACUNIT), finecosine[angle]);
        ore->momy = FixedMul((5 * FRACUNIT), finesine[angle]);
        ore->momz = FRACUNIT;
        return true;
    }
    else
...
boolean P_SpawnTeleportBeacon(player_t* player)
{
    ...
    if (P_CheckPosition(beacon, x, y))
    {
        ...
        beacon->momx = FixedMul((5 * FRACUNIT), finecosine[angle]);
        beacon->momy = FixedMul((5 * FRACUNIT), finesine[angle]);
