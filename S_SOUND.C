    ...
    sfxinfo_t*	sfxinfo)
{
    ...
    for (cnum = 0; cnum<numChannels; cnum++)
    {
        ...
        {
            if (sfxinfo->priority > channels[cnum].sfxinfo->priority)
                return -1;
            S_StopChannel(cnum);
            break;
        }
    ...
    int		volume)
{
    ...
    if (sfx_id < 1 || sfx_id > NUMSFX)
    {
        ...
        else if (!origin->sprite)
        {
            name = "SECTOR";
        }
        else
        {
            name = sprnames[origin->sprite];
        }
        I_Error("Bad sfx #: %d for obj %s", sfx_id, name);
    }
    ...
    if (sfx_id >= sfx_flburn && sfx_id <= sfx_flidl)
    {
        pitch += 8 - (M_Random() & 15);
...
void S_Start(void)
{
  ...
  if (gamemap > 31)
  {
      mnum = mus_action + gamemap - 32;
  }
  else
  {
      mnum = mus_action + gamemap - 1;
  }
  // HACK FOR COMMERCIAL
