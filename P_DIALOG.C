...
// multiplatform.
mapdialog_t *leveldialogs = NULL;

// The actual script00 dialogs. As above.
mapdialog_t *script0dialogs = NULL;

// Number of dialogs defined in the SCRIPT00 lump.
short numscript0dialogs;
...
int healthamounts[] = { -100 , -75, -50, -50, -100 };
...
dialogstateset_t dialogstatesets[] =
{
...
int numdialogstatesets = arrlen(dialogstatesets);

// Current dialog talker state
dialogstateset_t *dialogtalkerstates = NULL;
...
int numrndmessages = arrlen(rndMessages);
...
void P_DialogDrawer(void);

menuitem_t dialogmenuitems[] =
{
...
void P_DialogLoad(void)
{
    ...
    sprintf(lumpname, "script%02d", gamemap);
    lumpnum = W_CheckNumForName(lumpname);
    if (lumpnum != -1)
    {
        leveldialogs = W_CacheLumpNum(lumpnum, PU_LEVEL);
        numleveldialogs = W_LumpLength(lumpnum) / sizeof(mapdialog_t);
    }
    else
        numleveldialogs = 0;
    ...
    if (!script0loaded)
    {
        ...
        lumpnum = W_CheckNumForName("script00");
        script0dialogs = W_CacheLumpNum(lumpnum, PU_STATIC);
        numscript0dialogs = W_LumpLength(lumpnum) / sizeof(mapdialog_t);
    }
}
...
//
dialogstateset_t *P_DialogGetStates(mobjtype_t type)
{
    int i;
...
//
char *P_DialogGetMsg(char *message)
{
    int i;

    char *nameloc = message + 7;
...
boolean P_GiveInventoryItem(player_t *player, int sprnum, mobjtype_t type)
{
    ...
    for (curinv = 0; curinv <= player->numinventory; curinv++)
    {
...
boolean P_GiveItemToPlayer(player_t *player, int sprnum, mobjtype_t type)
{
    ...
    // check for keys
    if(type >= MT_KEY_BASE && type < MT_NEWKEY5 + 1)
    {
...
//
void P_TakeDialogItem(player_t *player, int type, int amount)
{
    int i;
...
void P_DialogDrawer(void)
{
    ...
    // draw the menu items
    for(i = 0, y = 0; i < dialogmenu.numitems - 1; i++, y += 19)
    {
        ...
        {
            sprintf(choicetext, "%s for %d", choicetext,
...
void P_DialogDoChoice(int choice)
{
    ...
    if(choice != dialogmenu.numitems - 1 && candochoice)
    {
        ...
        nextdialog = currentdialog->choices[choice].next;
        if(nextdialog != 0)
    ...
    if(choice != dialogmenu.numitems - 1)
    {
        ...
        objective = currentdialog->choices[choice].objective;
        if(objective)
        {
...
void P_DialogStart(player_t *player)
{
    ...
    else
    {
        ...
        if(mobjinfo[linetarget->type].name)
            ...
            dialogname = "Person"; // default name - like Joe in Doom 3 :P
    }
    ...
    {
    case 0:
        sprintf(dialoglastmsgbuffer, "%d) %s", i + 1, "BYE!");
        break;
    case 1:
        sprintf(dialoglastmsgbuffer, "%d) %s", i + 1, "Thanks, Bye!");
        break;
    case 2:
        sprintf(dialoglastmsgbuffer, "%d) %s", i + 1, "See you later!");
        break;
    }
    ...
    if(pic != -1)
        V_DrawPatchDirect(0, 0, 0, W_CacheLumpNum(pic, PU_CACHE));
